...
const Styles = (attributes: AttributesProps) => {
  ...
  const mainBlockSelector = '.editor-styles-wrapper .blogger-block-advancedheading'.concat(BLOCK_ID);
  const blockSelector = '.editor-styles-wrapper .blogger-block-advancedheading'.concat(BLOCK_ID);
  ...
  function replaceMediaQuery(device: string, allowDevice = false) {
    return ''.concat(
      ...
      addSelector(blockSelector, ''.concat(
        ...
        elseIf(allowDevice, addProperty(attr('color')?.includes('gradient') ? 'background' : 'color', attr('color'))),
        elseIf(allowDevice, attr('color')?.includes('gradient') ? '-webkit-background-clip: text;background-clip: text;-webkit-text-fill-color: transparent;' : ''),
        // elseIf(allowDevice,
  ...
  StyleSheets(`avd-heading_${blockId}`, ''.concat(
